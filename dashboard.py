...
class InventoryMS:
    ...
    def insert_sample_data(self):
        ...
        if self.cursor.fetchone()[0] == 0:
            products = [
                ('Monitor', 1, 10, 250,'Active'),
                ('Screen', 1, 0, 350,'Active'),
                ('PC', 2, 3, 800,'Active'),
                ('RAM', 2, 2, 120,'Inactive'),
                ('Keyboard', 1, 15, 50,'Inactive'),
                ('Mouse', 1, 20, 30,'Active'),
                ('Printer', 3, 8, 200,'Active'),
                ('Desk', 3, 7, 150,'Active')
            ]
            for product in products:
                self.cursor.execute("INSERT INTO products (name, category_id, stock, price,status) VALUES (?, ?, ?, ?,?)", product)
  ...
  #  def create_orders_page(self):
   ...
   #     self.pages['orders'] = page
    ...
    def load_products_data(self):
        ...
        self.cursor.execute("""
            SELECT p.id, p.name, c.name, p.stock,p.price,p.status
            FROM products p
            JOIN categories c ON p.category_id = c.id
            ORDER BY p.name
        """)
  ...
  #      """Load users data into the treeview"""
   ...
   # def load_profile_data(self):
    ...
    def show_add_product_dialog(self):
        ...
        status_dropdown = ttk.Combobox(dialog, textvariable=status_var, values=['Active', 'Inactive'], state='readonly')
        ...
        def save_product():
            ...
            if not name or not category_name or not stock or not price or not status :
            ...
            # Insert new product
            self.cursor.execute(
                "INSERT INTO products (name, category_id, stock, price,status) VALUES (?, ?, ?, ?, ? )",
                (name, category_id, stock, price,status)
            )
            self.conn.commit()
    ...
    def show_edit_product_dialog(self):
        ...
        product_id, name, category, stock, price, status  = item['values']
        ...
        def save_changes():
            ...
            if not new_name or not new_category or not new_stock or not new_price or not new_status :
                messagebox.showerror("Error", "All fields are required!")
                return
            ...
            # Update product
            self.cursor.execute(
                "UPDATE products SET name=?, category_id=?, stock=?, price=?,status=? WHERE id=?",
                (new_name, category_id, new_stock, new_price, new_status ,product_id)
            )
            self.conn.commit()
    ...
    def delete_product(self):
        ...
        item = self.products_tree.item(selected[0])
        product_id, name, _, _, _, _ = item['values']
