...
class BillClass:
    def __init__(self,root):
       ...
       # self.icon_title=PhotoImage(file="image\window.png")
        title=Label(self.root,text="Inventory Managemnet system",compound=LEFT,font=("times new roman",40,"bold"),bg="#010c48",fg="white",anchor="w",padx=20).place(x=0,y=0,relwidth=1,height=70)
       ...
       #=========Product Search Frame========
        ...
        self.product_Table=ttk.Treeview(ProductFrame3,columns=("id", "name", "stock", "price","status"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        self.product_Table.column("id",width=60)
...
#=================Customer Frame=====================================================
       ...
       #================== cart Frame==============
        ...
        self.CartTable=ttk.Treeview(Cart_Frame,columns=("id","name","stock","price"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        ...
        self.CartTable.column("stock",width=90)
       ...
       #==================ADD cart Button==============
        ...
        txt_p_name=Entry(Add_CartWidgetsFrame,textvariable=self.var_name,font=("times new roman",15),bg="lightyellow",state='readonly').place(x=5,y=30,width=190,height=25)
...
#======================= billing button================
    ...
    def show(self):
        ...
        try:
           ...
           cur.execute("Select id,name,price,stock,status from products where status='Active'")
           rows=cur.fetchall()
    ...
    def search(self):  
        ...
        try:
            if self.var_search.get()=="":  
                ...
                cur.execute("Select id,name,price,stock,status from products where name LIKE '%"+self.var_search.get()+"%' and status='Active'")
                rows=cur.fetchall()
                if len(rows)!=0:
                    self.product_Table.delete(*self.product_Table.get_children())
                    for row in rows:
    ...
    def add_update_cart(self):
        if  self.var_id.get()=='':
            messagebox.showerror('Error',"Please select products from the list",parent=self.root)
           ...
           #price_cal=float(price_cal)
            ...
            cart_data=[self.var_id.get(),self.var_name.get(),price_cal,self.var_qty.get(),self.var_stock.get()]
    ...
    def bill_top(self):  
        ...
        bill_top_temp=f'''
\t\tXYZ_Inventory
\tPhone No.98765*****,Nandurbar-125001
{str("="*40)}       
    ...
    def bill_middle(self): 
        ...
        try:   
            for row in self.cart_list:  
                ...
                cur.execute('Update products set stock=?,status=? where id=?',(
                    stock,  
                    status,
                    id      
                ))
